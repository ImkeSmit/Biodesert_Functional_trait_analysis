...
#Build model for Nintc ~ Fric

##Import filled FT data for the facilitation plots
FT_raw <- read.csv("Functional trait data\\FT_filled_match_facilitation_plots.csv", row.names = 1) 
...
###Loop through model predictor combinations ####
predictors <- c("FRic","FDiv", "FEve", "graz", "aridity")

# Generate combinations of predictor variables for models
predictor_combinations <- lapply(1:length(predictors), function(n) combn(predictors, n, simplify = TRUE))
predictor_combinations[[length(predictor_combinations) + 1]] <- matrix(c(1) ,nrow = 1, ncol = 1) #add an entry of 1 so that we can make a null model
...
for(r in 1:length(response_list)) {
  ...
  for (t in 1:length(predictor_combinations)) {
    comb <- predictor_combinations[[t]]
    
    columns <- ncol(comb)
    
    for (n in c(1:columns)) {
      
      predictors_formula <- paste(c( "(", paste(comb[, n], collapse = "*"), ")", collapse = "+"), collapse = "")
      formula <- as.formula(paste(response_var, "~", predictors_formula, "(1|site_ID)"))
      ...
      tryCatch( #tryCatch looks for errors and warinngs in the expression
        expr = {
          ...
          if (length(other_warnings) > 0) {
            ...
            message(paste("WARNING_", "r =" , response_var, "t =", t, "n =", n, warning_msg))
          }
        }, 
        
        #Also show me errors
        error = function(e) {
          message(paste("ERROR_", "r =" , response_var, "t =", t, "n =", n, conditionMessage(e)))
          print(e)
        }
      )
      ...
      result_row <- data.frame(Response = response_var,
                               Model = paste(response_var, "~", predictors_formula, "(1|site_ID)"), 
...
write.csv(results_table, "Functional trait data\\results\\model_results_16Feb2024.csv")
